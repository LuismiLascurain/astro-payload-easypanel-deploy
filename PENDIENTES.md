...
- required + localized: obligatorio en CADA idioma, y NOT NULL en la tabla
  _locales. Rompe el guardado de traducciones y se lleva por delante el texto
  recién escrito, sin aviso. Con fallback, obligatorio solo en el canónico
- El validate propio corre también en el navegador, y ahí req llega SIN locale:
  si no se sabe el idioma, no se bloquea
- admin.description no ve el documento (solo t e i18n): una ayuda que dependa
  del registro necesita components.Description
- Deducir la estructura de una página de la POSICIÓN de los nodos de un rich
  text: funciona hasta que el cliente escribe. El idioma y el nivel de
  encabezado se pueden neutralizar; el orden, no
